...
def github_merge_pr(values: dict[str, str], pr_number: int, commit_title: str | None) -> dict[str, Any]:
    repo = values["GITHUB_REPO"]
...
def render_trigger_deploy(values: dict[str, str]) -> dict[str, Any]:
...
def render_service(values: dict[str, str]) -> dict[str, Any]:
    service_id = values["RENDER_SERVICE_ID"]
    return request_json(
        "GET",
...
def render_list_deploys(values: dict[str, str], limit: int) -> list[dict[str, Any]]:
    service_id = values["RENDER_SERVICE_ID"]
    params = urllib.parse.urlencode({"limit": limit})
    return request_json(
        "GET",
        f"https://api.render.com/v1/services/{service_id}/deploys?{params}",
        headers=render_headers(values["RENDER_API_KEY"]),
    )
...
def vercel_list_deployments(values: dict[str, str], limit: int) -> dict[str, Any]:
...
def vercel_promote_target(values: dict[str, str], target: str) -> dict[str, Any]:
    project = vercel_project(values)
    project_id = project["id"]
...
def main() -> int:
    ...
    render_deploys = subparsers.add_parser("render-deploys")
    render_deploys.add_argument("--limit", type=int, default=5)
    ...
    try:
        ...
        if args.command in {"render-service", "render-deploy", "render-deploys"}:
        ...
        if args.command == "create-pr":
            data = github_create_pr(values, args.base, args.head, args.title, args.body)
            print(json.dumps({"number": data["number"], "url": data["html_url"]}, ensure_ascii=False, indent=2))
        elif args.command == "merge-pr":
            print(json.dumps(github_merge_pr(values, args.number, args.commit_title), ensure_ascii=False, indent=2))
        elif args.command == "render-service":
            print(json.dumps(render_service(values), ensure_ascii=False, indent=2))
        elif args.command == "render-deploy":
            print(json.dumps(render_trigger_deploy(values), ensure_ascii=False, indent=2))
        elif args.command == "render-deploys":
            print(json.dumps(render_list_deploys(values, args.limit), ensure_ascii=False, indent=2))
        elif args.command == "vercel-deployments":
            print(json.dumps(vercel_list_deployments(values, args.limit), ensure_ascii=False, indent=2))
        elif args.command == "vercel-redeploy":
            print(json.dumps(vercel_promote_target(values, args.target), ensure_ascii=False, indent=2))
        else:
